            ...
            try
            {
                ...
                Loaded += (sender, args) =>
                {
                    if (sp_Help.Children.Count == 0)
                    {
                        ...
                        sp_Help.Children.Add(new TextBlock() { Text = "Streams with DRM can turn your leds off because the video gets captured as a black screen due to the copy protection, a workaround in Chromium browsers is available by changing the angle graphics backend to DirectX 9, you can do this by typing 'chrome://flags/#use-angle' in your address bar and set the value to D3D9.", Style = (Style)App.Current.Resources["TextBlockGrayLight"], TextWrapping = TextWrapping.Wrap });
