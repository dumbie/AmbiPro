        ...
        void Load_Help_Text()
        {
            try
            {
                ...
                if (sp_Help_Text.Children.Count == 0)
                {
                    ...
                    sp_Help_Text.Children.Add(new TextBlock() { Text = "Streams with DRM can turn your leds off because the video gets captured as a black screen due to the copy protection, a workaround in Chromium browsers is available by changing the angle graphics backend to DirectX 9, you can do this by typing 'chrome://flags/#use-angle' in your address bar and set the value to D3D9.", Style = (Style)App.Current.Resources["TextBlockGrayLight"], TextWrapping = TextWrapping.Wrap });

                    sp_Help_Text.Children.Add(new TextBlock() { Text = "\r\nAmbiPro seems to impact my performance?", Style = (Style)App.Current.Resources["TextBlockBlack"] });
                    sp_Help_Text.Children.Add(new TextBlock() { Text = "AmbiPro captures your screen and based on that it creates calculations to see which leds need to be displayed and due to this it is a pretty resourceful application, but still optimized for the best performance possible.", Style = (Style)App.Current.Resources["TextBlockGrayLight"], TextWrapping = TextWrapping.Wrap });
